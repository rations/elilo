...
netfs_getinfo(netfs_interface_t *this, netfs_info_t *info)
{
	 ...
	 */
	if (nfs->using_pxe == 0) {
		ascii2U((CHAR8 *)nfs->pxe->Mode->DhcpAck.Dhcpv4.BootpBootFile, info->bootfile, NETFS_BOOTFILE_MAXLEN);
...
netfs_query_layer(netfs_interface_t *this, UINT16 server_type, UINT16 layer, UINTN maxlen, CHAR16 *str)
{
	...
	if(status == EFI_SUCCESS) {
		ascii2U((CHAR8 *)nfs->pxe->Mode->PxeReply.Dhcpv4.BootpBootFile, str, maxlen);
	} 
	return status;
}
