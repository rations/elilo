...
do_launch:
	...
	/*
	 * EFI Handover Protocol: preferred path for kernels >= 3.3 (boot protocol 2.12+).
	 * The kernel's EFI stub takes over and calls ExitBootServices itself.
...
check_edd30(VOID)
{
	...
	UINT8		edd30_val = FALSE;
	INTN		ret = -1;

	status = uefi_call_wrapper(RT->GetVariable, 5, L"EDD30", &edd30_guid, NULL, &l, &edd30_val);
	if (status == EFI_BUFFER_TOO_SMALL || (edd30_val != TRUE && edd30_val != FALSE)) {
		ERR_PRT((L"Warning: EDD30 EFI variable is not boolean value: forcing it to TRUE"));
		return -1;
	}
	if (status == EFI_SUCCESS && edd30_val == TRUE) {
...
force_edd30(VOID)
{
	...
	edd30_val = TRUE;
	status = uefi_call_wrapper(RT->SetVariable, 5, L"EDD30", &edd30_guid, EDD30_ATTR, l, &edd30_val);
	if (EFI_ERROR(status)) {
