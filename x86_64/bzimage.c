...
bzImage_alloc()
{
	...
	/* init_size (boot protocol offset 0x260 = pad_9[10]) may exceed the
	 * default 8 MB limit on modern 6.x kernels; honour it if larger. */
	init_sz = ps->s.pad_9[10];
	if (init_sz > (UINT32)kernel_size)
		kernel_size = (UINTN)init_sz;

	pages = EFI_SIZE_TO_PAGES(kernel_size);
	kend = kernel_start + kernel_size;
