...
static INTN get_video_info(boot_params_t * bp) {
	...
	if (EFI_ERROR(efi_status)) {
		Print(L"GOP not found, defaulting to text mode.\n"
		      L"Set text_mode=1 in elilo.conf to suppress this.\n");
		return -1;
	}
	...
	efi_status = uefi_call_wrapper(Gop_interface->QueryMode, 4,
				       ...
				       &size1, &Gop_info);
	if (EFI_ERROR(efi_status)) {
		ERR_PRT((L"GOP QueryMode failed."));
		return -1;
	}
	...
	bp->s.lfb_base   = (UINT32)(Gop_mode->FrameBufferBase & 0xFFFFFFFFULL);
	bp->s.lfb_size   = (UINT32)(Gop_mode->FrameBufferSize);
	bp->s.lfb_pages  = 1;
	bp->s.vesa_seg   = 0;
	bp->s.vesa_off   = 0;

	/* For framebuffers above 4 GB, store high 32 bits in ext_lfb_base
	 * (screen_info offset 0x3A = lfb_reserved[6]) and set
	 * VIDEO_CAPABILITY_64BIT_BASE (bit 1) in capabilities (offset 0x36 = lfb_reserved[2]). */
	if (Gop_mode->FrameBufferBase > 0xFFFFFFFFULL) {
		*(UINT32 *)(&bp->s.lfb_reserved[2]) |= 0x2U;
		*(UINT32 *)(&bp->s.lfb_reserved[6]) =
			(UINT32)(Gop_mode->FrameBufferBase >> 32);
	}
	...
	UINTN *cookie)
{
	...
	bp->s.loader_flags |= LDRFLAG_CAN_USE_HEAP;
